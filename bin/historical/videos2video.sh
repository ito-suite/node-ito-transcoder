...
# todo: use melt instead of burek's script

# gives a list of input files and the last is the output file
...
#WORKED! EXTRA_OPTIONS='-vcodec libx264 -crf 23 -preset medium -acodec aac -strict experimental -ac 2 -ar 44100 -ab 128k'
EXTRA_OPTIONS='-loglevel error -c:v libx264 -crf 23 -vprofile high -c:a aac -strict experimental -pix_fmt yuv420p -ac 2 -ar 44100 -ab 128k -vf hqdn3d=2:1:2'
...
/home/egal/ffmpeg_build/bin/ffmpeg -y -i $first -vn -f u16le -acodec pcm_s16le -ac 2 -ar 44100 $TMP/mcs_a1 2>/dev/null </dev/null &
/home/egal/ffmpeg_build/bin/ffmpeg -y -i $first -an -f yuv4mpegpipe -vcodec rawvideo $TMP/mcs_v1 2>/dev/null </dev/null &
...
do
    ...
    /home/egal/ffmpeg_build/bin/ffmpeg -y -i $f -vn -f u16le -acodec pcm_s16le -ac 2 -ar 44100 $TMP/mcs_a$i 2>/dev/null </dev/null &
    { /home/egal/ffmpeg_build/bin/ffmpeg -y -i $f -an -f yuv4mpegpipe -vcodec rawvideo - 2>/dev/null </dev/null | tail -n +2 > $TMP/mcs_v$i ; } &

    # if you need to log the output of decoding processes (usually not necessary)
    # then replace the "2>/dev/null" in 2 lines above with your log file names, like this:
    #ffmpeg -y -i $f -vn -f u16le -acodec pcm_s16le -ac 2 -ar 44100 $TMP/mcs_a$i 2>$TMP/log.a.$i </dev/null &
    #{ /home/egal/ffmpeg_build/bin/ffmpeg -y -i $f -an -f yuv4mpegpipe -vcodec rawvideo - 2>$TMP/log.v.$i </dev/null | tail -n +2 > $TMP/mcs_v$i ; } &
...
################################################################################
/home/egal/ffmpeg_build/bin/ffmpeg -f u16le -acodec pcm_s16le -ac 2 -ar 44100 -i $TMP/mcs_a_all \
